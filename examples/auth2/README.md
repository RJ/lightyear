# Auth with ConnectTokens via HTTP
...
In this example, the game server and the backend will run in the same process. The server will run a webserver
that answers requests to "/token-please" by generating a `ConnectToken`.

The client makes an http request to the "/token-please" endpoint, receives a `ConnectToken`, and uses it to
start the `lightyear` connection.
